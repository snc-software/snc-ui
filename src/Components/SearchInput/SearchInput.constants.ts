...
/**
 * Copied from `Autocomplete.constants.ts` rather than shared via `Internal/InputBase` — this
 * dropdown height scale isn't used by `Input`/`DatePicker`, so it doesn't clear the "shared by all"
 * extraction bar documented there.
 */
export const Sizes = {
  sm: cn('snc:h-8 snc:pl-9 snc:pr-2 snc:text-sm'),
  md: cn('snc:h-12 snc:pl-11 snc:pr-5 snc:text-sm'),
} as const;
