...
describe('TableBodyRow', () => {
  ...
  it('marks the row as current when isActive is true', () => {
    renderRow({ isActive: true });

    expect(screen.getByTestId('body-row-0')).toHaveAttribute('aria-current', 'true');
  });

  it('does not mark the row as current by default', () => {
    renderRow();

    expect(screen.getByTestId('body-row-0')).not.toHaveAttribute('aria-current');
  });
